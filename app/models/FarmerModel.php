...
class FarmerModel extends Model {
    ...
    public function getAllWithDetails(int $page = 1, int $perPage = 15, string $search = '', int $districtId = 0): array {
        $where = []; $params = [];
    ...
    public function getHarvestHistory(int $farmerId): array {
        $stmt = $this->db->prepare(
    ...
    public function getTotalHarvest(int $farmerId): float {
        $stmt = $this->db->prepare("SELECT COALESCE(SUM(quantity),0) FROM harvests WHERE farmer_id=?");
        $stmt->execute([$farmerId]);
        return (float) $stmt->fetchColumn();
    }

    public function getTopCrops(int $farmerId): array {
        $stmt = $this->db->prepare(
